...
const downloadAsLecturePptx = async (lecture: Data<LectureTwoLevelsDeep>) => {
  const pptxLectureObj = {
    lectureTitle: lecture.attributes.Title,
  ...
  const promises = pptxLectureObj.blocks.map(
    async (block: any, index: number) => {
      const slidePromises = block.attributes.Slides.map(
        async (slide: Slide, index: number) => {
          slide.id = slide.id.toString()

          return await convertSlideToMarkdown(slide, index)
        }
      )
  ...
  const pptxSlides = await Promise.all(promises)
  console.log(pptxSlides, 'pptxSlides')

  return pptxSlides
}

const convertSlideToMarkdown = async (
  slide: PptxSlide,
  index: number
): Promise<PptxSlide> => {
  ...
  const promises = Object.values(slide).map(async (value) => {
